...
router.get('/', async (req, res) => {
    ...
    const buscaDados = await BD.query(`SELECT * from categorias where upper(categorias.nome_categoria) like $1 order by ${ordenar}`, [`%${busca.toUpperCase()}%`])
    res.render('categoriasTelas/lista', {categorias: buscaDados.rows, busca, ordenar})}
    catch(erro){
...
router.get('/', async (req, res) => {
    try {
        ...
        res.status(500).render('categoriasTelas/lista', { mensagem: erro, categorias: [] })
    }
})
